...
public class DesktopWebviewWindowPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "setUserAgent":
      ...
      guard let userAgent = argument["userAgent"] as? String else {
        result(FlutterError(code: "0", message: "param userAgent not found", details: nil))
        return
      }
      wc.webViewController.setUserAgent(userAgent: userAgent)
      result(nil)
      break
    case "back":
      guard let argument = call.arguments as? [String: Any?] else {
